...
static void os_mmu_remove_all_mapping_l2(os_mmu_pt_l2_t *vtable)
{
    os_size_t i;
    ...
    for(i = 0;i < OS_MMU_L1_ENTRY_NUM;i++)
    {
        if(!__is_null_entry(vtable -> l1_vtable[i].value))
        {
            if(__is_pagetable(vtable -> l1_vtable[i].value))
            {
                ...
                os_mmu_remove_all_mapping_l2(next_vtable);
                os_memory_free((void *)OS_MMU_VA_TO_PA((os_size_t)next_vtable));
            }
        }
    }
...
void os_mmu_preinit()
{
    ...
    os_mmu_vtable_create(&kernel_pagetable,(os_mmu_pt_l1_p)&kernel_l1_pagetable,OS_MMU_MEMORYMAP_KERNEL_START,OS_MMU_MEMORYMAP_KERNEL_SIZE + OS_MMU_MEMORYMAP_IO_SIZE);
    os_mmu_create_mapping(&kernel_pagetable,OS_MMU_MEMORYMAP_KERNEL_START,MEMORY_BASE,DIV_UP(MEMORY_SIZE,OS_MMU_L1_SIZE) * OS_MMU_L1_SIZE,OS_MMU_PROT_KERNEL);

    os_mmu_vtable_create(&kernel_jump_pagetable,(os_mmu_pt_l1_p)&kernel_l1_jump_pagetable,OS_MMU_MEMORYMAP_USER_START,OS_MMU_MEMORYMAP_USER_SIZE + OS_MMU_MEMORYMAP_KERNEL_SIZE);
    os_mmu_create_mapping(&kernel_jump_pagetable,MEMORY_BASE,MEMORY_BASE,DIV_UP(MEMORY_SIZE,OS_MMU_L1_SIZE) * OS_MMU_L1_SIZE,OS_MMU_PROT_KERNEL);
    os_mmu_create_mapping(&kernel_jump_pagetable,OS_MMU_MEMORYMAP_KERNEL_START,MEMORY_BASE,DIV_UP(MEMORY_SIZE,OS_MMU_L1_SIZE) * OS_MMU_L1_SIZE,OS_MMU_PROT_KERNEL);
}
...
void os_mmu_preinit_secondary()
{
    ...
    os_mmu_vtable_remove(&kernel_jump_pagetable);
