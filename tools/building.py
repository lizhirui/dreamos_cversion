...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    # include kernel
    objs.extend(SConscript(Src_Root + '/src/SConscript', variant_dir=kernel_vdir + '/src', duplicate=0))
    # include arch
    objs.extend(SConscript(Src_Root + '/arch/SConscript',
                    variant_dir=kernel_vdir + '/arch', duplicate=0))
    # include firmware
    objs.extend(SConscript(Src_Root + '/firmware/SConscript',
                    variant_dir=kernel_vdir + '/firmware', duplicate=0))
