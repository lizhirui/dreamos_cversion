...
class Folder(object):
    ...
    def c_data(self, prefix=''):
        ...
        dhead = 'static const os_vfs_romfs_dirent_t %s[] = {\n' % (prefix + self.c_name)
        dtail = '\n};'
        body_fmt = '    {{.type = {type},.name = "{name}",.data = (os_uint8_t *){data},.size = sizeof({data})/sizeof({data}[0])}}'
        body_fmt0= '    {{.type = {type},.name = "{name}",.data = OS_NULL,.size = 0}}'
        # prefix of children
        ...
        for c in self._children:
            ...
            if isinstance(c, File):
                tp = 'OS_FILE_TYPE_REGULAR'
            elif isinstance(c, Folder):
                tp = 'OS_FILE_TYPE_DIRECTORY'
            else:
...
const os_vfs_romfs_dirent_t {name} = {{
    .type = OS_FILE_TYPE_DIRECTORY,.name = "/",.data = (os_uint8_t *){rootdirent},.size = sizeof({rootdirent})/sizeof({rootdirent}[0])
}};
'''
