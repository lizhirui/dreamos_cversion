...
task_t *current_task = OS_NULL;
os_bool_t need_lazy_task_switch = OS_FALSE;
task_t *lazy_old_task = OS_NULL;
task_t *lazy_next_task = OS_NULL;
...
//获取任务内核栈顶
os_size_t task_get_kernel_stack_top(task_t *task)
{
...
void task_scheduler_start()
{
    ...
    arch_task_switch(OS_NULL,&task_main);
}