...
// @formatter:off
//This file references some codes of kservice of rt-thread
...
static char *print_number(char *buf,char *end,long long num,int base,int s,int precision,int type)
{
    char c, sign;
    char tmp[32];
...
size_t os_vsnprintf(char *buf,size_t size,const char *fmt,va_list args)
{
    uint64_t num;
    int i, len;
    char *str, *end, c;
...
void os_printf(const char *fmt,...)
{
    va_list args;
    size_t length;
    static char buffer[OS_PRINTF_BUFFER_SIZE];

    va_start(args,fmt);
    length = os_vsnprintf(buffer,sizeof(buffer) - 1,fmt,args);
